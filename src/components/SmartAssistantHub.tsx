    ...
    <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '24px 16px', direction: 'rtl', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      {activeSection === 'routing' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '24px' }}>
          ...
          {/* Card C: Human AI Voice Settings (ElevenLabs / OpenAI / Azure) */}
          <div style={{
            background: 'rgba(15, 23, 42, 0.75)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ width: '38px', height: '38px', borderRadius: '10px', background: 'linear-gradient(135deg, rgba(236, 72, 153, 0.2), rgba(245, 158, 11, 0.2))', color: '#ec4899', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Sparkles size={20} />
                </div>
                <div>
                  <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#ffffff' }}>محرك الصوت البشري التوليدي (Human Voice AI)</h3>
                  <div style={{ fontSize: '0.75rem', color: '#94a3b8' }}>اختر مزود الذكاء الصوتي لتفعيل نبرة فتاة سعودية طبيعية 100% كأنها إنسان حقيقي</div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button
                  ...
                  onClick={() => {
                    unlockAudio();
                    speakSaudiFemaleVoice('يا هلا والله ومسهلا يا أبو سعود! هذا اختبار الصوت البشري الفائق لمؤسسة المحترز.. كيف تسمع النبرة الحين؟');
                  }}
                  style={{
                    padding: '8px 14px',
                    borderRadius: '10px',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '14px', marginBottom: '16px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.82rem', color: '#cbd5e1', fontWeight: 700, marginBottom: '6px' }}>
                  مزود الصوت الصوتي المفضل:
                </label>
                <select
                  value={typeof window !== 'undefined' ? (localStorage.getItem('muhtaraz_voice_provider') || 'auto') : 'auto'}
                  onChange={e => {
                    localStorage.setItem('muhtaraz_voice_provider', e.target.value);
                    alert('تم ضبط المزود بنجاح ✅');
                  }}
                  style={{
                    width: '100%',
                    padding: '10px 14px',
                ...
                >
                  <option value="auto" style={{ background: '#0f172a' }}>✨ تلقائي ذكي (Smart Auto)</option>
                  <option value="elevenlabs" style={{ background: '#0f172a' }}>🥇 ElevenLabs (صوت فتاة سعودية خارق الواقعية)</option>
                  <option value="openai" style={{ background: '#0f172a' }}>🥈 OpenAI TTS-1-HD (محرك شات جي بي تي)</option>
                </select>
              </div>

              <div>
                <label style={{ display: 'block', fontSize: '0.82rem', color: '#cbd5e1', fontWeight: 700, marginBottom: '6px' }}>
                  مفتاح API الخاص بك (API Key من ElevenLabs أو OpenAI):
                </label>
                <input
                  type="password"
                  defaultValue={typeof window !== 'undefined' ? (localStorage.getItem('muhtaraz_voice_api_key') || '') : ''}
                  onChange={e => {
                    localStorage.setItem('muhtaraz_voice_api_key', e.target.value.trim());
                  }}
                  placeholder="ألصق مفتاح ElevenLabs أو OpenAI هنا..."
                  style={{
                    width: '100%',
                    padding: '10px 14px',
                    borderRadius: '10px',
            ...
            <div style={{ fontSize: '0.75rem', color: '#64748b' }}>
              💡 بمجرد لصق المفتاح المجاني من موقع <strong>ElevenLabs.io</strong>، يتحول الصوت فوراً وبأعلى جودة إلى صوت بشري لا يمكن تفرقته عن إنسان حقيقي!
            </div>
          </div>
        </div>
      )}
